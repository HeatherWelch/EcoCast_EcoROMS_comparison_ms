...
master=master %>% mutate(EcoROMS_original=rescale(EcoROMS_original,to=c(0,1))) %>% mutate(Marxan_raw=rescale(Marxan_raw,to=c(0,1)))
...
# EcoA <- within(EcoA, Ocean_state[Year == '2003'] <- 'Cold') %>% mutate(EcoROMS_original_unscaled=rescale(EcoROMS_original_unscaled,to=c(0,1))) %>% mutate(Marxan_raw_unscaled=rescale(Marxan_raw_unscaled,to=c(0,1)))
...
# EcoC=read.csv("hindcast_ms/extract/extractions/run_C.3.csv") %>% mutate(weighting="-0.5/0.5")%>% mutate(weighting=as.factor(weighting)) %>% mutate(Year=as.factor(substr(dt,1,4)))%>% mutate("Ocean_state"="Warm")
...
# EcoC=do.call("rbind",list(EcoC, EcoC.1, EcoC.2)) %>% mutate(EcoROMS_original_unscaled=rescale(EcoROMS_original_unscaled,to=c(0,1))) %>% mutate(Marxan_raw_unscaled=rescale(Marxan_raw_unscaled,to=c(0,1)))

C1=ggplot(EcoC,aes(x=swor,y=EcoROMS_original,group=weighting,color=weighting,shape=Ocean_state))+geom_point(aes(alpha=Ocean_state),size=1)+stat_smooth(se=F,method = "gam",formula = y~s(x, bs="cr"))+
...
C2=ggplot(EcoC,aes(x=swor,y=Marxan_raw,group=weighting,color=weighting,shape=Ocean_state))+geom_point(aes(alpha=Ocean_state),size=1)+stat_smooth(se=F,method = "gam",formula = y~s(x, bs="cr"))+
  scale_alpha_manual("Ocean State",values = c("Warm"=.3,"Cold"=1))+
  scale_color_manual("Weighting",values=c("+/-0.1"="darkgoldenrod","-0.3/0.7"="darkgoldenrod","-0.5/0.5"="coral1","-0.7/0.3"="dimgray","+/-0.9"="dimgray"))+
  scale_shape_manual("Ocean State",values=c("Warm"=3,"Cold"=15))+
  theme(text = element_text(size=5),axis.text = element_text(size=5),plot.title = element_text(hjust=0,size=5),legend.position=c(.1,.95),legend.justification = c(.9,.9),legend.key.size = unit(.5,'lines'))+
...
C3=ggplot(EcoC,aes(x=lbst,y=EcoROMS_original,group=weighting,color=weighting,shape=Ocean_state))+geom_point(aes(alpha=Ocean_state),size=1)+stat_smooth(se=F,method = "gam",formula = y~s(x, bs="cr"))+
  scale_alpha_manual("Ocean State",values = c("Warm"=.3,"Cold"=1))+
  scale_color_manual("Weighting",values=c("+/-0.1"="darkgoldenrod","-0.3/0.7"="darkgoldenrod","-0.5/0.5"="coral1","-0.7/0.3"="dimgray","+/-0.9"="dimgray"))+
  scale_shape_manual("Ocean State",values=c("Warm"=3,"Cold"=15))+
...
C4=ggplot(EcoC,aes(x=lbst,y=Marxan_raw,group=weighting,color=weighting,shape=Ocean_state))+geom_point(aes(alpha=Ocean_state),size=1)+stat_smooth(se=F,method = "gam",formula = y~s(x, bs="cr"))+
  scale_alpha_manual("Ocean State",values = c("Warm"=.3,"Cold"=1))+
  scale_color_manual("Weighting",values=c("+/-0.1"="darkgoldenrod","-0.3/0.7"="darkgoldenrod","-0.5/0.5"="coral1","-0.7/0.3"="dimgray","+/-0.9"="dimgray"))+
  scale_shape_manual("Ocean State",values=c("Warm"=3,"Cold"=15))+
...
png(paste0(plotdir_ms,"figure2_changing_weightings_temp3.1_scaled.png"),width=10, height=8, units="in", res=400)
par(ps=10)
par(cex=1)
par(mar=c(4,4,1,1))
...
file=paste0(plotdir_ms,"figure2_changing_weightings_temp3.1_scaled.png")
...
a
image_write(a,path = paste0(plotdir_ms,"figure2_changing_weightings_temp3.1_scaled.png"))
