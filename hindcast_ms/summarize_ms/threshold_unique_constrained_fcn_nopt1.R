...
thresholds_unique_constrained_nopt1=function(csvdir,plotdir,lbstRisk,cons_obj1,cons_obj2,cons_obj3,type){
...
dff=df %>% mutate(swor_blsh_casl=blueshark+casl+swor_inverse) %>% mutate(swor_blsh=blueshark+swor_inverse) %>% mutate(swor2=swor_inverse)
...
png(paste0(plotdir,"parellel_coordinate_plot_thresholds_",lbstRisk,"_lbst_unique_constrained_",type,"_nopt1.png"),width=14, height=7, units="in", res=400)
par(ps=10)
par(cex=1)
par(mar=c(4,4,1,1))
...
write.csv(c,paste0(plotdir,"parellel_coordinate_table_thresholds_",lbstRisk,"_lbst_unique_constrained_",type,"_nopt1.csv"))
...
png(paste0(plotdir,"parellel_coordinate_boxplot_thresholds_",lbstRisk,"_lbst_unique_constrained_",type,"_nopt1.png"),width=7, height=3.5, units="in", res=400)
par(ps=10)
par(cex=1)
par(mar=c(4,4,1,1))
print({a})
