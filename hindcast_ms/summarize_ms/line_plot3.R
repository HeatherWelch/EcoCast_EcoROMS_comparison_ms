...
b=b %>% filter(Product!="EcoROMS_original_unscaled")
...
aa=ggplot(b, aes(x = variable, y = value, group = id)) +   # group = id is important!
  ...
  scale_color_manual("Product",values=c("EcoROMS original"="darkgoldenrod","Marxan raw"="coral1","Marxan raw unscaled"="aquamarine4"))+
...
png(paste0(plotdir,"parellel_coordinate_plot_10_new_mgmt_no_E_OUns.png"),width=14, height=7, units="in", res=400)
par(ps=10)
par(cex=1)
par(mar=c(4,4,1,1))
...
write.csv(c,paste0(plotdir,"parellel_coordinate_table_10_no_E_OUns.csv"))
