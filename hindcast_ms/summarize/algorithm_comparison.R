...
agorithm_comparison=function(data,weightings,outdir,run){
  ...
  for(i in 1:length(catch)){ 
    binA=data %>% select(-c(X,lon,lat,dt))%>% gather(species,suitability,-c(EcoROMS_original,Marxan_raw,EcoROMS_original_unscaled,Marxan_raw_unscaled)) %>% filter(suitability>=.5) %>% spread(species,suitability) %>% select(EcoROMS_original,Marxan_raw,EcoROMS_original_unscaled,Marxan_raw_unscaled,swor) %>% .[complete.cases(.),]
  ...
  for(i in 1:nrow(empty)){
    tablecaughtE=data %>% select(-c(X,lon,lat,dt))%>% gather(species,suitability,-c(EcoROMS_original,Marxan_raw,Marxan_raw_unscaled,EcoROMS_original_unscaled)) %>% gather(product,product_value,-c(species,suitability)) %>% dplyr::filter(suitability>=.5) %>% dplyr::filter(product_value>=empty$EcoROMS_original[i]) %>% 
      dplyr::filter(product=="EcoROMS_original")%>% group_by(species,product) %>% summarise(num_presences_caught=n()) 
    
    tablecaughtM=data %>% select(-c(X,lon,lat,dt))%>% gather(species,suitability,-c(EcoROMS_original,Marxan_raw,Marxan_raw_unscaled,EcoROMS_original_unscaled)) %>% gather(product,product_value,-c(species,suitability)) %>% dplyr::filter(suitability>=.5) %>% dplyr::filter(product_value>=empty$Marxan_raw[i]) %>%
      dplyr::filter(product=="Marxan_raw")%>% group_by(species,product) %>% summarise(num_presences_caught=n())  %>% rbind(.,tablecaughtE)
    
    tablecaughtEuS=data %>% select(-c(X,lon,lat,dt))%>% gather(species,suitability,-c(EcoROMS_original,Marxan_raw,Marxan_raw_unscaled,EcoROMS_original_unscaled)) %>% gather(product,product_value,-c(species,suitability)) %>% dplyr::filter(suitability>=.5) %>% dplyr::filter(product_value>=empty$EcoROMS_original_unscaled[i]) %>%
    ...
    tablecaughtMEuS=data %>% select(-c(X,lon,lat,dt))%>% gather(species,suitability,-c(EcoROMS_original,Marxan_raw,Marxan_raw_unscaled,EcoROMS_original_unscaled)) %>% gather(product,product_value,-c(species,suitability)) %>% dplyr::filter(suitability>=.5) %>% dplyr::filter(product_value>=empty$Marxan_raw_unscaled[i]) %>%
      dplyr::filter(product=="Marxan_raw_unscaled")%>% group_by(species,product) %>% summarise(num_presences_caught=n())  %>% rbind(.,tablecaughtEuS)
    
    tabletotal=data %>% select(-c(X,lon,lat,dt))%>% gather(species,suitability,-c(EcoROMS_original,Marxan_raw,Marxan_raw_unscaled,EcoROMS_original_unscaled)) %>% gather(product,product_value,-c(species,suitability)) %>% dplyr::filter(suitability>=.5) %>%
  ...
  a=ggplot()+geom_line(data=sworDat %>% filter(product=="EcoROMS_original"),aes(x=swor,y=lbst,color="EcoROMS_original"),size=.5)
  a=a+geom_line(data=sworDat %>% filter(product=="Marxan_raw"),aes(x=swor,y=lbst,color="Marxan_raw"),size=.8,linetype=2)
  a=a+geom_line(data=sworDat %>% filter(product=="EcoROMS_original_unscaled"),aes(x=swor,y=lbst,color="EcoROMS_original_unscaled"),size=.5)
  a=a+geom_line(data=sworDat %>% filter(product=="Marxan_raw_unscaled"),aes(x=swor,y=lbst,color="Marxan_raw_unscaled"),size=.8,linetype=2)
  ...
  colnames(empty)=c("availcatch","Marxan_raw","EcoROMS_original","Marxan_raw_unscaled","EcoROMS_original_unscaled")
  for(i in 1:length(catch)){ 
    binA=data %>% select(-c(X,lon,lat,dt))%>% gather(species,suitability,-c(EcoROMS_original,Marxan_raw,EcoROMS_original_unscaled,Marxan_raw_unscaled)) %>% filter(suitability>=.5) %>% spread(species,suitability) %>% select(EcoROMS_original,Marxan_raw,EcoROMS_original_unscaled,Marxan_raw_unscaled,lbst) %>% .[complete.cases(.),]
  ...
  ## plot ####
  subtitle=paste0(namesrisk[1],": ",weightings[1],", ",namesrisk[2],": ",weightings[2],", ",namesrisk[3],": ",weightings[3],", ",namesrisk[4],": ",weightings[4],", ",namesrisk[5],": ",weightings[5])
  a=ggplot()+geom_line(data=lbstDat %>% filter(product=="EcoROMS_original"),aes(x=lbst,y=swor,color="EcoROMS_original"),size=.5)
  a=a+geom_line(data=lbstDat %>% filter(product=="Marxan_raw"),aes(x=lbst,y=swor,color="Marxan_raw"),size=.8,linetype=2)
  a=a+geom_line(data=lbstDat %>% filter(product=="EcoROMS_original_unscaled"),aes(x=lbst,y=swor,color="EcoROMS_original_unscaled"),size=.5)
  a=a+geom_line(data=lbstDat %>% filter(product=="Marxan_raw_unscaled"),aes(x=lbst,y=swor,color="Marxan_raw_unscaled"),size=.8,linetype=2)
