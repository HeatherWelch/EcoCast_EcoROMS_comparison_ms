...
c=master %>% filter(run=="run_B.2"|run=="run_B.3"|run=="run_B.4"|run=="run_C.2"|run=="run_C.3"|run=="run_C.4")  %>%select(-EcoROMS) %>%  spread(run,Marxan)%>% rename(Marxan_B.2=run_B.2)%>% rename(Marxan_B.3=run_B.3)%>% rename(Marxan_B.4=run_B.4)%>% rename(Marxan_C.2=run_C.2)%>% rename(Marxan_C.3=run_C.3)%>% rename(Marxan_C.4=run_C.4) %>% select(-c(lat,lon,dt))
...
c=master %>% filter(run=="run_B.2"|run=="run_B.3"|run=="run_B.4"|run=="run_C.2"|run=="run_C.3"|run=="run_C.4")  %>%select(-EcoROMS) %>%  spread(run,Marxan) %>% rename("M: lbst (-.3)"=run_B.2)%>% rename("M: lbst (-.5)"=run_B.3)%>% rename("M: lbst (-.7)"=run_B.4) %>% rename("M: lbst/swor (-.3/.3)"=run_C.2)%>% rename("M: lbst/swor (-.5/.5)"=run_C.3)%>% rename("M: lbst/swor (-.7/.7)"=run_C.4) %>% select(-c(lat,lon,dt))
