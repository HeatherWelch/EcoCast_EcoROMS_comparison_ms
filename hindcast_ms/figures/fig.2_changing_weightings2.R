...
EcoA=read.csv("hindcast_ms/extract/extractions/run_M.1.csv") %>% mutate(weighting=1)%>% mutate(weighting=as.factor(weighting)) %>% mutate(Year=as.factor(substr(dt,1,4)))
EcoB=read.csv("hindcast_ms/extract/extractions/run_M.2.csv") %>% mutate(weighting=-1)%>% mutate(weighting=as.factor(weighting)) %>% mutate(Year=as.factor(substr(dt,1,4)))
EcoC=read.csv("hindcast_ms/extract/extractions/run_C.3.csv") %>% mutate(weighting="+/-0.5")%>% mutate(weighting=as.factor(weighting)) %>% mutate(Year=as.factor(substr(dt,1,4)))

## A
A1=ggplot(EcoA,aes(x=swor,y=EcoROMS_original_unscaled,group=weighting,color=weighting,shape=Year))+geom_point(aes(alpha=Year),size=1)+
...
## B
B1=ggplot(EcoB,aes(x=lbst,y=EcoROMS_original_unscaled,group=weighting,color=weighting,shape=Year))+geom_point(aes(alpha=Year),size=1)+
  scale_alpha_manual("Year",values = c("1997"=.3,"2003"=.6,"2005"=.3))+
  scale_color_manual("Weighting",values=c("-1"="black"))+
  scale_shape_manual("Year",values=c("1997"=3,"2003"=15,"2005"=4))+
...
## C
C1=ggplot(EcoC,aes(x=swor,y=EcoROMS_original_unscaled,group=weighting,color=weighting,shape=Year))+geom_point(aes(alpha=Year),size=1)+stat_smooth(se=F)+
  scale_alpha_manual("Year",values = c("1997"=.3,"2003"=.6,"2005"=.3))+
  scale_color_manual("Weighting",values=c("+/-0.1"="darkgoldenrod","+/-0.3"="cornflowerblue","+/-0.5"="coral1","+/-0.7"="aquamarine4","+/-0.9"="dimgray"))+
  scale_shape_manual("Year",values=c("1997"=3,"2003"=15,"2005"=4))+
  theme(text = element_text(size=5),axis.text = element_text(size=5),plot.title = element_text(hjust=0,size=5),legend.position=c(.07,.95),legend.justification = c(.9,.9),legend.key.size = unit(.5,'lines'))+
...
C3=ggplot(EcoC,aes(x=lbst,y=EcoROMS_original_unscaled,group=weighting,color=weighting,shape=Year))+geom_point(aes(alpha=Year),size=1)+stat_smooth(se=F)+
  scale_alpha_manual("Year",values = c("1997"=.3,"2003"=.6,"2005"=.3))+
  scale_color_manual("Weighting",values=c("+/-0.1"="darkgoldenrod","+/-0.3"="cornflowerblue","+/-0.5"="coral1","+/-0.7"="aquamarine4","+/-0.9"="dimgray"))+
  scale_shape_manual("Year",values=c("1997"=3,"2003"=15,"2005"=4))+
  theme(text = element_text(size=5),axis.text = element_text(size=5),plot.title = element_text(hjust=0,size=5),legend.position=c(.07,.3),legend.justification = c(.9,.9),legend.key.size = unit(.5,'lines'))+
...
png(paste0(plotdir_ms,"figure2_changing_weightings.png"),width=20, height=8, units="in", res=400)
par(ps=10)
par(cex=1)
par(mar=c(4,4,1,1))
...
file=paste0(plotdir_ms,"figure2_changing_weightings.png")
...
a
image_write(a,path = paste0(plotdir_ms,"figure2_changing_weightings.png"))
