...
b=master %>% filter(run=="run_M.2"|run=="run_C.3") %>%select(-Marxan) %>%  spread(run,EcoROMS) %>%rename(EcoCast_M.2=run_M.2)%>%rename(EcoCast_C.3=run_C.3)%>% select(-c(lat,lon,dt))
c=master %>% filter(run=="run_B.1"|run=="run_B.2"|run=="run_B.3"|run=="run_B.4"|run=="run_B.5"|run=="run_C.1"|run=="run_C.2"|run=="run_C.3"|run=="run_C.4"|run=="run_C.5")  %>%select(-EcoROMS) %>%  spread(run,Marxan)%>%  rename(Marxan_B.5=run_B.5)%>%rename(Marxan_B.1=run_B.1)%>% rename(Marxan_B.2=run_B.2)%>% rename(Marxan_B.3=run_B.3)%>% rename(Marxan_B.4=run_B.4) %>% rename(Marxan_C.5=run_C.5)%>%   rename(Marxan_C.1=run_C.1)%>% rename(Marxan_C.2=run_C.2)%>% rename(Marxan_C.3=run_C.3)%>% rename(Marxan_C.4=run_C.4) %>% select(-c(lat,lon,dt))
a=cbind(b,c[,5:ncol(c)]) %>% .[complete.cases(.),] %>% select(-c(Sealion,Blueshark)) %>% .[,c(1:2,4,3,5:ncol(.))]
a=cor(a)
...
b=master %>% filter(run=="run_M.2"|run=="run_C.3") %>%select(-Marxan) %>%  spread(run,EcoROMS) %>%rename("E: lbst (1)"=run_M.2)%>%rename("E: lbst/swor (-.5/.5)"=run_C.3)%>% select(-c(lat,lon,dt))
c=master %>% filter(run=="run_B.1"|run=="run_B.2"|run=="run_B.3"|run=="run_B.4"|run=="run_B.5"|run=="run_C.1"|run=="run_C.2"|run=="run_C.3"|run=="run_C.4"|run=="run_C.5")  %>%select(-EcoROMS) %>%  spread(run,Marxan)%>%  rename("M: lbst (-.9)"=run_B.5)%>%rename("M: lbst (-.1)"=run_B.1)%>% rename("M: lbst (-.3)"=run_B.2)%>% rename("M: lbst (-.5)"=run_B.3)%>% rename("M: lbst (-.7)"=run_B.4) %>% rename("M: lbst/swor (-.9/.9)"=run_C.5)%>% rename("M: lbst/swor (-.1/.1)"=run_C.1)%>% rename("M: lbst/swor (-.3/.3)"=run_C.2)%>% rename("M: lbst/swor (-.5/.5)"=run_C.3)%>% rename("M: lbst/swor (-.7/.7)"=run_C.4) %>% select(-c(lat,lon,dt))
a=cbind(b,c[,5:ncol(c)]) %>% .[complete.cases(.),] %>% select(-c(Sealion,Blueshark)) %>% .[,c(1:2,4,3,5:ncol(.))]
a=cor(a)
png(paste0(plotdir_ms,"figure3_changing_weightings_cor.png"),width=12, height=8, units="in", res=400)
par(ps=10)
