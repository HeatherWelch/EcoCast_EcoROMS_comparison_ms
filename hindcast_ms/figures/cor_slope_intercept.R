...
for(i in 1:length(runs)){
  runn=runs[i]
  a=master %>% filter(run==runn) %>% select(-c(run,dt,y_m)) %>% cor() %>% .[1:2,3:6] %>% as.data.frame() %>% mutate(algorithm=c("EcoROMS","Marxan")) %>% mutate(run=runn) %>% mutate(stat="Cor")
...
write.csv(d,paste0(plotdir,"cor_slope_intercept.csv"))
